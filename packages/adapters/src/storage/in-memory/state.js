import { createContextItem, createContextVersion, } from '@ledgermind/domain';
const cloneContextItemRef = (item) => {
...
export const cloneContextItem = (item, position = item.position) => {
...
export const cloneArtifactContent = (content) => {
    if (content === null) {
        return null;
    }
...
export const createInMemoryPersistenceState = () => {
    return {
...
export const cloneInMemoryPersistenceState = (state) => {
    return {
        conversations: new Map(state.conversations),
        nextConversationOrdinal: state.nextConversationOrdinal,
        ledgerEventsByConversation: new Map([...state.ledgerEventsByConversation.entries()].map(([conversationId, events]) => [
            conversationId,
            [...events],
        ])),
        ledgerEventsById: new Map(state.ledgerEventsById),
        contextItemsByConversation: new Map([...state.contextItemsByConversation.entries()].map(([conversationId, items]) => [
            conversationId,
        ...
        summaryNodeIdsByConversation: new Map([...state.summaryNodeIdsByConversation.entries()].map(([conversationId, summaryIds]) => [
            conversationId,
            [...summaryIds],
        ])),
        leafMessageEdgesBySummary: new Map([...state.leafMessageEdgesBySummary.entries()].map(([summaryId, messageIds]) => [
            summaryId,
            [...messageIds],
        ])),
        condensedParentEdgesBySummary: new Map([...state.condensedParentEdgesBySummary.entries()].map(([summaryId, parentSummaryIds]) => [
            summaryId,
            [...parentSummaryIds],
        ])),
        artifactsById: new Map([...state.artifactsById.entries()].map(([artifactId, record]) => [
            artifactId,
            {
...
export const applyInMemoryPersistenceState = (target, source) => {
    ...
    for (const [conversationId, items] of source.contextItemsByConversation.entries()) {
        target.contextItemsByConversation.set(conversationId, items.map((item) => cloneContextItem(item)));
    }
...
export const getContextVersionOrDefault = (state, conversationId) => {
