...
| **Factory.ai**       | Probe-based evaluation measures _functional_ quality (can the agent continue?), not just lexical similarity. Structure in summaries forces preservation.            |
...
**P1 (next):** 6. multi-round-condensation-lineage 7. postgres-restart-rehydration-parity
...
it('previously appended events are never modified', () => {
  fc.assert(
    fc.asyncProperty(arbitraryEventSequence({ minLength: 2, maxLength: 20 }), async (events) => {
      const engine = createTestEngine();
...
it('returned objects are not internal references (no mutation leaks)', () => {
  fc.assert(
    fc.asyncProperty(arbitraryEventSequence({ minLength: 3, maxLength: 10 }), async (events) => {
...
These tests verify _structural_ correctness, not semantic quality:
...
jobs:
  quality:
    steps:
      ...
      - pnpm --filter @ledgermind/domain test # Pure unit tests (~60% of tests)
  ...
  postgres:
    ...
    steps:
      - pnpm vitest run tests/conformance # Conformance against PostgreSQL
```
...
jobs:
  extended:
    steps:
      - pnpm vitest run tests/regression # Full regression catalog
      - pnpm --filter @ledgermind/sdk test # SDK E2E tests

  llm-eval:
    steps:
      - pnpm vitest run tests/quality # LLM-as-judge and QA recall evaluation
      - pnpm vitest run tests/probes # Probe-based continuation evaluation
...
## Agent Continuity Probes

LedgerMind includes coding-agent continuity probes under `tests/probes`.

These probes validate behavior generic long-memory benchmarks do not cover:

- session resume from a handoff after reset
- active decision recall after stale or superseded decisions
- failed verification remembered as blocking evidence
- tool evidence returned with command and file provenance
