...
**Implemented through:** HLD Phase 1 → SDK, PostgreSQL, SQLite local durability, MCP, Claude Code hooks, and golden/conformance/regression gates.
...
| 5   | SDK entrypoint (`createMemoryEngine()` factory)                                                                          | ✅ Complete | `sdk`                     |
| 6   | Golden/conformance/regression suites                                                                                     | ✅ Complete | `tests`                   |
...
| SQLite adapter for local durable continuity                            | ✅ Complete | `infrastructure/sqlite`      |
| MCP server package                                                     | ✅ Complete | `mcp-server`                 |
| Claude Code hooks                                                      | ✅ Complete | `claude-code`                |

### ❌ Remaining Phase 1 polish

| #   | HLD Phase 1 Deliverable                                 | Status         | Notes                                                                                            |
| --- | ------------------------------------------------------- | -------------- | ------------------------------------------------------------------------------------------------ |
| 5   | Basic tokenizer (simple estimator + tiktoken)           | ⚠️ Partial     | Simple tokenizer exists; tiktoken binding not yet                                                |
| 6   | 5 core explorers (TS, Python, JSON, Markdown, Fallback) | ⚠️ Partial     | Fallback plus implemented core explorers should be reconciled against current package coverage   |
| 8   | Vercel AI SDK adapter                                   | ❌ Not started | No framework tool adapters exist                                                                 |
| 9   | Golden test suite expansion                             | ⚠️ Partial     | Golden suite exists for in-memory/PostgreSQL; SQLite is covered by conformance and restart smoke |
...
### Sprint 7 — Local Durability & Hardening (2 weeks)

> **Goal:** Harden embedded local storage and production-readiness.

| #   | Task                                                        | Package                 | Depends On | Deliverable                                                                                                 |
| --- | ----------------------------------------------------------- | ----------------------- | ---------- | ----------------------------------------------------------------------------------------------------------- |
| 7.1 | SQLite adapter (persistence ports)                          | `infrastructure/sqlite` | —          | ✅ Complete via `node:sqlite`, mirror-table/LIKE search, recursive expansion, local single-writer semantics |
| 7.2 | Contract tests: same suite runs PG + SQLite + in-memory     | `tests`                 | 7.1        | ✅ Complete for conformance; golden/probe remain in-memory + PostgreSQL                                     |
| 7.3 | `createMemoryEngine({ storage: { type: 'sqlite', path } })` | `sdk`                   | 7.1        | ✅ Complete local durable memory                                                                            |
| 7.4 | MCP server `--storage sqlite --sqlite <path>` support       | `mcp-server`            | 7.3        | ✅ Complete local-first Claude Code setup                                                                   |
